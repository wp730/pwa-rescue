...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `domains` | `string[]` | **是** | 备份域名数组,按优先级排序。第一个是默认推荐域(用户看到的"Baixe app"按钮指向)。元素必须是完整 URL 含 `https://`,末尾斜杠可有可无。**长度 ≥ 1**,返回空数组视为失败。<br><br>**前端会自动给域名补 `www.` 前缀**:`https://321322222.cc` → `https://www.321322222.cc`。已经是 `www.` 开头或纯 IP 地址的会跳过,所以后端可以直接传裸域,也可以直接传 www 子域,都能正常工作。 |
